...
irqreturn_t dsa_complete(int irq, void* data)
{
	struct idxd_irq_entry *ie = data;

	//complete(&ie->done);
	if (ie->ksm_wait_queue) {
		ie->done = 1;
		wake_up_interruptible(ie->ksm_wait_queue);
	}
...
int idxd_wq_request_irq(struct idxd_wq *wq)
{
	...
	rc = request_irq(ie->vector, dsa_complete, 0, "idxd-portal", ie);
	if (rc < 0) {
